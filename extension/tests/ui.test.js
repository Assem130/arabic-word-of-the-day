...
function element() {
  return {
    textContent: "", hidden: false, disabled: false, checked: false, value: "", dataset: {}, attributes: {},
    classList: { add() {}, remove() {}, toggle() {} },
    setAttribute(name, value) { this.attributes[name] = String(value); },
    addEventListener(type, listener) { this.listeners[type] = listener; }, listeners: {}, focus() {},
  };
}

function popupApi() {
  const elements = new Map();
  const ids = ["status", "onboarding", "assigned", "empty", "recovery", "warning", "word", "meaning-ar", "meaning-en", "example", "pronunciation", "fixed-label", "save", "speak", "reminder", "onboarding-submit", "onboarding-skip", "explore", "known", "difficult"];
  ...
  const calls = [];
  const extension = {
    runtime: { sendMessage(message) { calls.push(message); return Promise.resolve({}); }, getURL(value) { return `extension://kalimat/${value}`; } },
